...
pressure=['200','400','800','1000','3000'] #columns need to isolate the first 0,0 data
gas=['air','alcohol','CO','CH4','LPG','H2']
data=pd.read_csv('MQ7_analysis\\MQ7_dataset.csv')
data=data[pressure]
data=data.dropna(axis='columns') #remove column data but the lavels still there
data.index=[gas]
print(data)
...
dominant_sensor= pd.Series(pca.components_[0], index=[pressure])
